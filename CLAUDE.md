...
### 坐标来源：OpenStreetMap 实名地物

三站与三个起点的坐标取自 **OSM 实名地物**（WGS-84，与运行时同坐标系），不是现场实测。
换点位时不要靠看瓦片数格子——那次南门偏了 **262 米**，而解锁半径只有 30 米。取法：
...
# 2) 再用 Nominatim 补单个 POI；高德瓦片只用来**肉眼复核落点**，不用来取数
```

已知实名坐标备查：西二门 30.754845,103.921800（node/2271615612）／主楼 30.749073,103.925117
（way/687370083）／南门 30.746551,103.922913（node/2271614699）／成都合院 30.746403,103.918538
（way/543052123）／龙湖时代天街购物广场 30.754618,103.920084／时间广场 30.751917,103.927456。
⚠️ OSM 是社区测绘，**出发前仍要用 geo-test.html 在三处各站一次**。

### 插画：公版，装金框

`public/assets/art/` 下两张公主插画是 **Edmund Dulac 的公版作品**（黄金时代童话插画，
迪士尼那套视觉的直系祖先），来源与授权见同目录 `CREDITS.json`。暖色调画面直接铺在夜空上
会打架，所以一律装进 `.royal-plate` 烫金画框当"一页彩插"——金框是 App 既有的视觉语言。
